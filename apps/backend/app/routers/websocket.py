...
async def websocket_endpoint(websocket: WebSocket, token: str):
    ...
    try:
        ...
        if deriv_ws:
            # Set up Deriv message handlers
            deriv_ws.add_message_handler("tick", lambda data: handle_tick_data(data, user.id))
            deriv_ws.add_message_handler("portfolio", lambda data: handle_portfolio_data(data, user.id))
            deriv_ws.add_message_handler("buy", lambda data: handle_buy_response(data, user.id))
            deriv_ws.add_message_handler("sell", lambda data: handle_sell_response(data, user.id))

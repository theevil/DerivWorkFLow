  ...
  if (!data || data.length === 0) {
    return (
      <div className="card-elevated">
        <h3 className="text-xl font-bold text-retro-brown-700 mb-4">
          {title}
        </h3>
  ...
  return (
    <div className="card-elevated">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-retro-brown-700">
          {title}
        </h3>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            <div 
              className={`w-3 h-3 rounded-full ${color === 'danger' ? 'bg-retro-red-500' : 'bg-retro-turquoise-500'}`}
            ></div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        <div className="neumorph-inset p-4 text-center rounded-2xl">
          <p className="text-xs text-retro-brown-500 uppercase tracking-wide mb-1">Highest</p>
          <p className="font-bold text-retro-brown-700">
            ${maxValue.toFixed(2)}
          </p>
        </div>
        <div className="neumorph-inset p-4 text-center rounded-2xl">
          <p className="text-xs text-retro-brown-500 uppercase tracking-wide mb-1">Average</p>
          <p className="font-bold text-retro-brown-700">
            ${(data.reduce((sum, point) => sum + point.value, 0) / data.length).toFixed(2)}
          </p>
        </div>
        <div className="neumorph-inset p-4 text-center rounded-2xl">
          <p className="text-xs text-retro-brown-500 uppercase tracking-wide mb-1">Lowest</p>
          <p className="font-bold text-retro-brown-700">
            ${minValue.toFixed(2)}
          </p>
        </div>

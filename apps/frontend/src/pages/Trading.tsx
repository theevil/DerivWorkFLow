...
const contractTypes: ContractType[] = [
  {
    value: 'CALL',
    label: 'Higher',
    description: 'Predict that the market will end higher',
  },
  {
    value: 'PUT',
    label: 'Lower',
    description: 'Predict that the market will end lower',
  },
];
...
export function TradingPage() {
  ...
  useWebSocketEvent(
    ...
    useCallback(
      (tickData: any) => {
        ...
        const changePercent =
          currentData.tick > 0 ? (change / currentData.tick) * 100 : 0;
  ...
  useWebSocketEvent(
    ...
    useCallback((response: any) => {
      ...
      if (response.data && response.data.buy) {
        ...
        alert(
          `Contract purchased successfully! Contract ID: ${response.data.buy.contract_id}`
        );
  ...
  useEffect(() => {
    if (
      isConnected &&
      selectedSymbol &&
      !subscribedSymbols.has(selectedSymbol)
    ) {
                    ...
                    <div className='flex justify-between items-center'>
                      ...
                      <div className='text-right'>
                        <div className='text-lg font-bold text-gray-900 dark:text-white'>
                          {formatPrice(
                            getCurrentMarketData(selectedSymbol)!.tick
                          )}
                        </div>
                        <div
                          className={`text-sm ${
                ...
                <Button
                  ...
                  className={
                    selectedContract === 'CALL'
                      ? 'btn-primary bg-success-600 hover:bg-success-700'
                      : 'btn-primary bg-danger-600 hover:bg-danger-700'
                  }
                  size='lg'
                >
                  {loading
                    ? 'Processing...'
                    : `Buy ${selectedContract} for $${amount}`}
                </Button>

                {/* Emergency Stop Button - Only show if auto trading is enabled */}
                {isAutoTradingEnabled && (
                  <div className='mt-4 pt-4 border-t border-gray-200 dark:border-gray-700'>
                    <EmergencyStopButton size='sm' variant='light' fullWidth />
                  </div>
                )}
              </div>
            </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            <div className='p-4 bg-primary-50 dark:bg-primary-900/20 rounded-lg'>
              ...
              <p className='text-sm text-primary-700 dark:text-primary-300'>
                Never risk more than you can afford to lose. Start with small
                amounts.
              </p>
            </div>
